...
use "$my_outputdir/cod_ab1_2019.dta", clear 
tsset month l_in_bin
 xtline count
 graph export "$my_outputdir/cod_ab1.png", as(png) replace


gen cod_fish_weight=$kilo_to_lbs*$cod_lwa*((l_in_bin+$lngcat_offset_cod)/$cm_to_inch)^$cod_lwb
...
browse
pause
use "$my_outputdir/atlanticcod_b2_counts_2019.dta", clear 
...
tsset month l_in_bin
 xtline b2_count
 graph export "$my_outputdir/cod_b2.png", as(png) replace
...
browse month cl cw b2_count length weight   if year==2019
browse
pause


use "$my_outputdir/haddock_ab1_2019.dta", clear 


gen haddock_fish_weight=$kilo_to_lbs*$had_lwa*((l_in_bin+$lngcat_offset_haddock)/$cm_to_inch)^$had_lwe
gen cl=count*l_in_bin
gen cw=count*haddock_fish_weight
tsset month l_in_bin
 xtline count
 graph export "$my_outputdir/haddock_ab1.png", as(png) replace
...
use "$my_outputdir/haddock_b2_counts_2019.dta", clear 
...
tsset month l_in_bin
 xtline b2_count
 graph export "$my_outputdir/haddock_b2.png", as(png) replace
