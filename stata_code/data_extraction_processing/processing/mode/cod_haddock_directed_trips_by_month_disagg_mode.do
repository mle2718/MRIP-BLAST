...
gen str mode="HE" if inlist(mode_fx,4)
replace mode="CH" if inlist(mode_fx,5)
replace mode="PR" if inlist(mode_fx,7) 
replace mode="SH" if inlist(mode_fx,1,2,3) 
...
gen my_dom_id_string=year2+area_s+"_"+mode + "_"+month+"_"+dom_id
...
keep my_dom_id year area_s month mode dom_id
...
save "${my_outputdir}/`my_common1'_`my_common2'_target_disagg_mode_${working_year}.dta", replace
