...
This excludes trips that Targeted Cod but did not (catch cod, catch haddock or target haddock)
...
replace dom_id="2" if strmatch(prim1_common, "`my_common1'") /*replace all trips that targeted cod into dom=2 */
...
format dtrip %10.0fc
save "$my_outputdir/`my_common2'_plus_target_${working_year}.dta", replace
